...
const archiveHashes = new Set(['#archive', '#characters', '#about'])
const getViewFromHash = () =>
  archiveHashes.has(window.location.hash) ? 'archive' : 'landing'
...
function App() {
  ...
  useEffect(() => {
    const handleHashChange = () => {
      ...
      if (window.location.hash === '#archive' || window.location.hash === '#home') {
  ...
  return (
    <div className={`page-shell ${isArchive ? 'archive-view' : 'landing-view'}`}>
      <header className="site-header">
        <a className="brand" href={isArchive ? '#home' : '#top'} aria-label="返回首页">
          <span>RE</span>
          <i />
        ...
        {isArchive ? (
          ...
          <p className="header-status">RACCOON CITY INCIDENT · CLASSIFIED</p>
        )}
      </header>
